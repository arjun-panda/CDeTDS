# CDeTDS Release Script
# Usage: .\release.ps1 -Version 1.0.1 -Notes "Bug fixes and improvements"
# This script bumps version in CDeTDS.App.csproj and CHANGELOG.txt together.
...
# -- 2. Bump CDeTDS.App.csproj ------------------------------------------------
Step "Bumping CDeTDS.App.csproj"
$csprojPath = Join-Path $root "CDeTDS.App\CDeTDS.App.csproj"
...
# -- 4. Build -----------------------------------------------------------------
Step "Building"
dotnet build "$root\CDeTDS.App\CDeTDS.App.csproj" -c Debug --nologo 2>&1 | Select-Object -Last 3
if ($LASTEXITCODE -ne 0) { Fail "Build failed" }
Ok "Build passed"

# -- 5. Test ------------------------------------------------------------------
Step "Running tests"
dotnet test "$root\CDeTDS.Tests\CDeTDS.Tests.csproj" --nologo 2>&1 | Select-Object -Last 3
...
dotnet publish "$root\CDeTDS.App\CDeTDS.App.csproj" -c Release -r win-x64 --self-contained -o $publishDir --nologo 2>&1 | Select-Object -Last 3
...
vpk pack -u CDeTDS -v $Version -p $publishDir --packTitle "CDeTDS" -e CDeTDS.exe -o $releaseDir
...
git -C $root add "CDeTDS.Common\AppConstants.cs" "CDeTDS.App\CDeTDS.App.csproj" "CHANGELOG.txt"
