...
Computed under {(d.Regime == "Old" ? "Old" : "New")} Tax Regime · FY {d.FinancialYear} · Generated by TDS Pro v{TDSPro.Common.AppConstants.AppVersion}
</p>
</body></html>";
        }
        ...
        public static string SavePdf(IncomeComputationData d, string outputDir)
        {
            ...
            byte[] pdf = PdfReports.BuildA4(
                ...
                body:     c => c.Column(col =>
                {
                    ...
                    col.Item().PaddingTop(20).AlignCenter().Text($"Computed under {(d.Regime == "Old" ? "Old" : "New")} Tax Regime · FY {d.FinancialYear}").FontSize(9).Italic().FontColor(PdfReports.MutedColor);
                }));
